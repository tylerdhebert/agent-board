...
export function DiffModal({ cardId, cardTitle, branchName, onClose }: DiffModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-4xl mx-4 bg-[#111118] border border-[#2a2a38] rounded-sm shadow-2xl flex flex-col max-h-[90vh]"
      ...
      >
        ...
        {/* Stat summary */}
        {!loading && data?.stat && (
          <div className="px-4 py-2 border-b border-[#1e1e2a] bg-[#0d0d14]">
            <pre className="text-[11px] font-mono text-[#94a3b8] whitespace-pre-wrap">
              {data.stat.trim()}
            </pre>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && !error && data && (
            <pre className="p-4 font-mono text-xs leading-relaxed bg-[#0a0a0f] overflow-x-auto">
              {data.diff.length === 0 ? (
                <span style={{ color: "#94a3b8" }}>No differences found.</span>
              ) : (
                data.diff.split("\n").map((line, i) => (
                  <span key={i} style={{ color: colorDiffLine(line), display: "block" }}>
                    {line || "\u00a0"}
                  </span>
                ))
              )}
